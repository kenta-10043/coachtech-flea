...
@section('content')
    <div class="profile__container">
        <img class="profile__image" src="{{ asset('storage/' . $buyerUser->profile->profile_image) }}"
            alt="{{ $buyerUser->name }}">
        <p class="main__content__title">{{ $buyerUser->name }}さん111との取引画面</p>
    </div>
    ...
    <div class="chat__area">
            ...
            @foreach ($myMessages as $myMessage)
                @if (!empty($myMessage->body))
                    <div class="chat__profile__container">
                        ...
                        <img class="chat__profile__image" src="{{ asset('storage/' . $buyerUser->profile->profile_image) }}"
                            alt="{{ $user->name }}">
                    </div>
        ...
        @foreach ($partnerMessages as $partnerMessage)
            @if (!empty($partnerMessage->body))
                <div class="chat__profile__container">
                    <img class="chat__profile__image" src="{{ asset('storage/' . $sellerUser->profile->profile_image) }}"
                        alt="{{ $sellerUser->name }}">
